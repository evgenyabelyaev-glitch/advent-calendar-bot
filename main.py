...
# Пытаемся установить локаль, если не выходит — просто игнорируем, 
# так как на Render часто нет предустановленных локалей
try:
    ...
    try:
        ...
        logging.warning("Русская локаль не установлена. Используется стандартное форматирование.")
...
# ИСПРАВЛЕНО: Убраны лишние вложенные скобки, вызывавшие ошибку
ADVENT_MESSAGES = {
    3: {
        ...
        3: ("✨ День 2: Фото-охота (Задание на 50 баллов!)\n\n"
        ...
        4: ("🌿 День 3: Кулинарный интуитив (Задание на 40 баллов!)\n\n"
        ...
        5: ("🍬 День 4: Музыкальная пауза (Задание на 30 баллов!)\n\n"
           "Пришли мне песню, которая у тебя ассоциируется со мной или с началом наших отношений.\n\n"
           "Мое послание: Хочу, чтобы ты знала — ты самая нежная и хрупкая, и я всегда буду твоей опорой. Ты — моя мелодия. 🎶"),
        
        6: ("🎈 День 5: Минутка планов (Задание на 60 баллов!)\n\n"
           "Придумай одно любое безумное или очень спокойное занятие, которое мы обязательно сделаем вместе этой весной.\n\n"
           "Мое послание: Я ценю твое терпение и мудрость. Мы пройдем через любые сложности, потому что мы — команда. Обожаю тебя! 🗺️💪"),
        
        7: ("🌙 День 6: Предвкушение (Задание на 20 баллов!)\n\n"
...
get_message_button = types.InlineKeyboardMarkup(
    inline_keyboard=[[types.InlineKeyboardButton(text="Открыть весенний сюрприз 🌸", callback_data="get_advent_message")]]
)
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "С началом весны! 🌱\nКаждый день до 8 марта тебя ждет послание и задание, за которое ты будешь получать баллы. 8 марта откроется магазин, где ты сможешь их потратить! 🎁",
...
async def process_advent_callback(callback: types.CallbackQuery):
    ...
    if str(callback.from_user.id) != ADMIN_ID:
        try:
            await bot.send_message(ADMIN_ID, f"🔔 Соня открыла календарь! Сегодня {today_day} марта. Жди выполнения задания! 🌸")
        except: pass

    if today_month == 3 and today_day in ADVENT_MESSAGES[3]:
        msg = ADVENT_MESSAGES[3][today_day]
        # Используем ручное форматирование даты, если локаль не сработала
        months_ru = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"]
        date_str = f"{today_day} {months_ru[today_month-1]}"
        text = f"🗓️ {html.bold(f'Послание на {date_str}:')}\n\n{msg}"
    elif today_month == 3 and today_day < 2:
        text = "⏳ Твой весенний марафон начнется 2 марта. Готовься копить баллы! 🌱"
    else:
        text = "😴 Новое задание появится завтра утром! Хорошего отдыха. ✨"
...
async def forward_all_messages(message: types.Message):
    ...
    try:
        await bot.send_message(ADMIN_ID, f"📩 Ответ/выполнение задания от пользователя {message.from_user.full_name}:")
...
async def main():
    try:
        ...
        await site.start()
        # Для Render важно поддерживать процесс активным
