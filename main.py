...
ADVENT_MESSAGES = {
    3: {
        2: "🌸 Начинаем подготовку к празднику! И сегодня у нас мини-викторина: 1. Чем пахла кошка, когда ее только взяли? 2. Какое самое первое колечко я тебе подарил? 3. Какой у тебя был телефон, когда мы познакомились? А чтобы лучше думалось, немного приятных слов: я очень горжусь, что у меня такая женщина - у тебя много увлечений, ты добрая и заботливая, даже в такой сложный для нас период. Я тебя люблю",
...
get_message_button = types.InlineKeyboardMarkup(
    inline_keyboard=[[types.InlineKeyboardButton(text="Открыть сюрприз 🌸", callback_data="get_advent_message")]]
)
